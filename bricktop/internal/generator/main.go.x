...
import (
	...
	"github.com/azmodb/bricktop/tool/parser"
)

func main() {
	var (
		proto = []string{"github.com", "azmodb", "bricktop", "tool", "protocol"}
...
//
const ( {{range $k, $v := .}}
	_{{toUpper .Type}} Type = {{typeNum .Type}}{{end}}
)
...
func (t Type) String() string {
	switch t { {{range $k, $v := .}}
	case _{{toUpper .Type}}:
		return "{{toLower .Type}}" {{end}}
	}
	return fmt.Sprintf("unknown 9P2000.L fcall type (%d)", t)
}

func marshal(buf *Buffer, typ Type, tag uint16, v Fcall) error {
	switch f := v.(type) { {{range $k, $v := .}}
	case *{{toPublic .Type}}:
		{{toWireGo .}}	
		return buf.Err() {{end}}
	}
...
func unmarshal(buf *Buffer, typ Type, tag uint16) (Fcall, error) {
	switch typ { {{range $k, $v := .}}
	case _{{toUpper .Type}}:
		f := &{{toPublic .Type}}{tag: tag}
		{{fromWireGo .}}	
		return f, buf.Err() {{end}}
	}
	return nil, fmt.Errorf("unknown 9P2000.L fcall type (%d)", typ)
}
...
func (f {{toPublic .Type}}) Fcall() Type { return _{{toUpper .Type}} }
func (f {{toPublic .Type}}) Tag() uint16       { return f.tag }
//func (f {{toPublic .Type}}) String() string    { return "TODO" }
...
uint32_t fcall_unmarshal(unsigned char*, uint32_t, fcall_t*);
uint32_t fcall_marshal(unsigned char*, uint32_t, fcall_t*);
uint32_t fcall_size(fcall_t*);

#endif /* __FCALL_H */
`

	cTmpl = `#include "libc.h"

uint32_t
fcall_unmarshal(unsigned char* data, uint32_t msize, fcall_t* f)
{
	if(msize < 7)
		return 0;
...
uint32_t
fcall_marshal(unsigned char* data, uint32_t msize, fcall_t* f)
{
	if(msize < 7)
		return 0;
...
var gomain = "$GOPATH/github.com/azmodb/tool/generator"
